...
(define test-list
  '(
    ...
    (proc-7 "(let (f (lambda (x) (set x 4))) (let (a 30) (begin (f a) a))))" 30)
    (proc-8 "(let (f (lambda (x) (set x 4))) (let (a 30) (begin (f (+ a 1)) a)))" 30)
    (proc-9 "(let (f (lambda (x) (set x (+ x 4)))) (let (a 30) (begin (f a) a))))" 30)
    ...
    (set-1 "(let (x 1) (begin (set x 2) x))" 2)
    (set-2 "(let (x 1) (begin (set x 2) (set x 3) x))" 3)
    (set-3 "(let (x 1) (begin (set x (+ x 5)) x))" 6)
    ))

...
class RAGDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.is_train:
            ...
            #### YOUR CODES; TODO 
            positive_ctxs = inputs.get('positive_ctxs', [])
            if len(positive_ctxs) == 0:
                raise ValueError(f"No positive context found for uid {uid}")

            ctx = random.choice(positive_ctxs)
            title = ctx.get('title') or ''
            passage = ctx.get('text') or ctx.get('contents') or ctx.get('passage') or ''
